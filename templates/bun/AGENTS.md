...
- This repository is a Bun-first template with optional UV-backed Python quality gates.
- The current scaffold is intentionally minimal (`src/`, `tests/`, `scripts/` contain placeholders until features are added).
...
- Formatting:
  - `bun run format`
- When Python tooling is enabled, prefer locked UV execution (for example, `uv sync --locked`, `uv run --locked ruff check`, `uv run --locked pytest`).
...
- `.agents/instructions/tooling-bun-uv.instructions.md` — Bun contracts and optional UV enablement guidance
