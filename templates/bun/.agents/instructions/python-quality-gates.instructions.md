...
- Apply these rules when Python scaffolding is enabled (for example, `pyproject.toml` exists or Python modules are being added).
...
If present, keep policy tests (for example `tests/test_module_exports.py` and `tests/test_docstrings.py`) aligned with these rules.
